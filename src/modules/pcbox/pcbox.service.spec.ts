...
describe('PcboxService', () => {
  function buildService(overrides: { verify?: jest.Mock } = {}) {
    const verify = overrides.verify ?? jest.fn().mockResolvedValue(undefined);
    ...
    const ticketHubVerificationService = {
      verify,
    } as unknown as TicketHubVerificationService;
    const ansibleService = { execute } as unknown as AnsibleService;

    const service = new PcboxService(
      administrationsRepository,
      ticketHubVerificationService,
      ansibleService,
    );

    return { service, verify, execute, createAdministration };
  }

  it('gate 1: rejects a non-APPROVED status before calling ticket-hub-api at all', async () => {
    const { service, verify, execute, createAdministration } = buildService();
  ...
  it('gate 2: propagates a ticket-hub-api verification failure before parsing YAML or saving', async () => {
    const verify = jest
      .fn()
      .mockRejectedValue(new Error('ticket-hub-api says no'));
    const { service, execute, createAdministration } = buildService({
      verify,
    });

    await expect(
      service.create(buildDto({ fileContent: 'this is: [ not, valid, yaml' })),
    ).rejects.toThrow('ticket-hub-api says no');

    expect(execute).not.toHaveBeenCalled();
    expect(createAdministration).not.toHaveBeenCalled();
  });

  it('gate 3: rejects unparseable YAML after ticket-hub-api already matched, before saving', async () => {
    const { service, verify, execute, createAdministration } = buildService();
  ...
  it('happy path: all three gates pass, saves the record, then executes the playbook', async () => {
    const { service, verify, execute, createAdministration } = buildService();
